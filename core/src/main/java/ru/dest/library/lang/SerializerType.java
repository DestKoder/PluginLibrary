...
public enum SerializerType {
    ...
    LEGACY(() -> {
        ...
        try {
            Optional<URLClassLoader> opt = RuntimeDependency.loadIfAbsent("net.kyori.adventure.text.serializer.legacy.LegacyComponentSerializer", "https://dk-develop.ru/lib/kyori-serializer/legacy");
            if(opt.isPresent()){
    ...
    MINI_MESSAGE(() -> {
        ComponentSerializer<? extends Component, ? extends Component, String> serializer;
        try {
            Optional<URLClassLoader> opt = RuntimeDependency.loadIfAbsent("net.kyori.adventure.text.minimessage.MiniMessage", "https://dk-develop.ru/lib/kyori-serializer/minimsg");
            if(opt.isPresent()) {
