...
//        if(mat == null) throw new InvalidMaterialException(data[0]);
        ...
        Item item = ItemsRegistry.current().byId(new RegistryKey(data[0], data[1]));
        if(item == null) throw new IllegalArgumentException("Unknown item "+data[0] + ":" + data[1]);
        ItemStack itemStack = item.get();

        if(data.length == 3 && data[2].matches(Patterns.INTEGER)){
            itemStack.setAmount(Integer.parseInt(data[2]));
        }
        return itemStack;
    }
    ...
    public ItemStack fromJson(JsonObject object){
        ...
        if(itemId.startsWith("minecraft:")){
            item = new ItemStack(Material.getMaterial(itemId.split(":")[1].toUpperCase()));
        }else{
            cItem = ItemsRegistry.current().byId(RegistryKey.fromString(itemId));
            item = cItem.get();
        }
        ...
        if(cItem != null) return cItem.fromJson(item, object);
        else return item;
    }
